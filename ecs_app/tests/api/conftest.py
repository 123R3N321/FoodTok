...
from .endpoint_coverage import EndpointCoverageTracker


def pytest_configure(config: Any) -> None:
    tracker = EndpointCoverageTracker().start()
    config._endpoint_tracker = tracker


def pytest_unconfigure(config: Any) -> None:
    tracker = getattr(config, "_endpoint_tracker", None)
    if tracker is None:
        return
    tracker.stop()
...
def pytest_terminal_summary(terminalreporter: Any) -> None:
    tracker = getattr(terminalreporter.config, "_endpoint_tracker", None)
    if tracker is None:
        return
    ...
    write(f"Tracked endpoints : {summary['total']}\n")
    write(f"Hit during run    : {summary['hit']}\n")
    write(f"Coverage          : {summary['coverage']:.1f}%\n")
    ...
    if missing:
        ...
        write("Missing endpoints : none 🎉\n")

    unknown = summary["unknown"]
    if unknown:
        write("Unknown endpoints :\n")
